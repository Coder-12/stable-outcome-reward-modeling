...
def corrupt_steps(steps: List[str]):
    """Apply multi-stage corruptions to reasoning steps (safe version)."""
    new_steps = []
    all_ops = []
    ...
    if len(new_steps) == 0:
        new_steps = ["(Corrupted reasoning step omitted.)"]
        all_ops.append("insert_fallback_step")
        return new_steps, all_ops

    # --- Random truncation (safe) ---
    if random.random() < 0.20:
        if len(new_steps) > 1:
            low = max(1, len(new_steps) // 2)
            high = len(new_steps)
            if low < high:
                keep = random.randint(low, high)
